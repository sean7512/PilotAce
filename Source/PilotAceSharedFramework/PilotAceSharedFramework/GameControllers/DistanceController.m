...
+ (NSTimeInterval)determineStationaryObjectDurationFromPositionX:(CGFloat)xPos withWidth:(CGFloat)width {
    ...
    NSTimeInterval duration = pixelsToMove / (PLANE_MOVE_PIXELS_PER_SECOND * [[GameSettingsController sharedInstance].nodeScaleDelegate getNodeScaleSize]);
    return duration;
}
...
+ (NSTimeInterval)determineMovingObjectDuration:(SKSpriteNode *)node withAdditionalSpeed:(CGFloat)percentOfPlaneSpeed {
    ...
    NSTimeInterval duration = pixelsToMove / (PLANE_MOVE_PIXELS_PER_SECOND * percentOfPlaneSpeed * [[GameSettingsController sharedInstance].nodeScaleDelegate getNodeScaleSize]);
    return duration;
}
