...
+ (id)createForDraggable:(AllowableDragDirection)dragDirection {
    ...
    dispatch_once(&loadRaptorTextureOnce, ^{
        UIImage *image = [UIImage imageNamed:PLANE_IMG inBundle:[NSBundle bundleForClass:[RaptorPlane class]] compatibleWithTraitCollection:nil];
        _raptorTexture = [SKTexture textureWithImage:image];
    });
